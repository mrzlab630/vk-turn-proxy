...
Проброс трафика WireGuard/Hysteria через TURN сервера VK звонков или legacy/best-effort flow Яндекс Телемоста. Пакеты шифруются DTLS 1.2, затем параллельными потоками через TCP или UDP отправляются на TURN сервер по протоколу STUN ChannelData. Оттуда по UDP отправляются на ваш сервер, где расшифровываются и передаются в WireGuard. Для VLESS-режима TCP-соединения дополнительно мультиплексируются через KCP + smux поверх DTLS. Логин/пароль от TURN генерируются из ссылки на звонок.
...
**UPD. ТЕЛЕМОСТ ЗАКРЫЛИ.** Код получения TURN-кредов для Яндекса оставлен как legacy/best-effort flow и может сломаться при изменениях приватных API. Текущая схема зафиксирована в [docs/API_FLOWS.md](docs/API_FLOWS.md).
